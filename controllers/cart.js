...
exports.addToCart = async (req, res, next) => {
    try {
        ...
        const cart = await Cart.findOne({ product: product_id, user: req.user._id });
        
        if (cart) {
            cart.quantity += quantity;
            await cart.save();
        } else {
            await Cart.create({ product: product_id, quantity, user: req.user._id });
        }
        ...
        if (
            ...
            res.locals.response = HttpResponse.successResponse({ total: totalItemsInCart });
            return next();
        }
    } catch (error) {
